...
const EditBill = () => {
  ...
  const baseURLBill = `http://localhost:8080/api/v1/bill/${id}`;
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      await axios
        ...
        .then((resp) => {
          console.log(resp);
          setFromDate(formatDate(resp.data.data.fromDate));
          setToDate(formatDate(resp.data.data.toDate));
          setElectricC(resp.data.data.electricConsumption);
          setWaterC(resp.data.data.waterConsumption);
          if (resp.data.data.dateOfPayment != null) {
            setDateOfPayment(formatDate(resp.data.data.dateOfPayment));
          }

          setTotalPrice(resp.data.data.totalPrice);
          setApartment(resp.data.data.apartment.id);

          var list = [];
          resp.data.data.bill_details.map((item) => {
            if (item.service_fee.id == 1) {
              setWaterP(item.service_fee.unitPrice);
            }
            if (item.service_fee.id == 2) {
              setElectricP(item.service_fee.unitPrice);
            }
            list.push(item.service_fee.id);
          });
          setListService(list);
  ...
  const electricExpense = () => {
    if (listService.includes(2) || electric == true) {
      return (
  ...
  const waterExpense = () => {
    if (listService.includes(1) || water == true) {
      return (
        <div className={styles.fl}>
          <div className={styles.l}>
        ...
        <div className={styles.root}>
          ...
          <div className={styles.info}>
            <div>
              ...
              <div className={styles.fl}>
                <div className={styles.l}>
                  <p className={styles.lable}> Date Of Payment</p>
                </div>
                <div className={styles.r}>
                  <input
                    type="Date"
